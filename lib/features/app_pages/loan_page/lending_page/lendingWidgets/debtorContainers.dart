...
import 'lendingViewDetails/lendingMoreInfos.dart';
...
class DebtorList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        children: debtors
            .map((debtor) => _buildDebtorCard(
          context,
          debtor: debtor, // ✅ pass the full map
        ))
            .toList(),
      ),
    );
  ...
  Widget _buildDebtorCard(
      BuildContext context, {
        required Map<String, dynamic> debtor,
      }) {
    final dark = UHelperFunctions.isDarkMode(context);

    final name = debtor['name'];
    final date = debtor['date'];
    final amount = debtor['amount'];
    final status = debtor['status'];
    final color = debtor['color'] as Color;
    final initials = debtor['initials'];
